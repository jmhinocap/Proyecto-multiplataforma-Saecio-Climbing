...
function comprobarValor(str, max) {
...
fecha.addEventListener('input', function(e) {
  ...
  var valores = input.split('/').map(function(v) {
    return v.replace(/\D/g, '')
  });
  if (valores[0]) valores[0] = comprobarValor(valores[0], 31);
  if (valores[1]) valores[1] = comprobarValor(valores[1], 12);
  var output = valores.map(function(v, i) {
...
fecha.addEventListener('blur', function(e) {
  this.type = 'text';
  var input = this.value;
  var valores = input.split('/').map(function(v, i) {
    return v.replace(/\D/g, '')
  });
  var output = '';
  
  if (valores.length == 3) {
    var agno = valores[2].length !== 4 ? parseInt(valores[2]) + 2000 : parseInt(valores[2]);
    var mes = parseInt(valores[0]) - 1;
    var dia = parseInt(valores[1]);
    var d = new Date(agno, mes, dia);
    if (!isNaN(d)) {
